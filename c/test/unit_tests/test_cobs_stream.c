...
// output buffer for testing encoded output
uint8_t             out_buffer[1000];

// need to keep track of output buffer write position
int out_buf_idx = 0;

// writes bytes to output buffer
static size_t write(const uint8_t* data, size_t len)
{
    int i = 0;
    for (; i < len; i++)
        out_buffer[out_buf_idx++] = data[i];
    return i;
}

// reset the output buffer write index
static void _vResetOutputBuf()
{
    out_buf_idx = 0;
}
...
TEST_GROUP(CobsSteamTests);

TEST_SETUP(CobsSteamTests)
{
